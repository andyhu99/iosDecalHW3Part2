...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    let postDict: [String:AnyObject] = ["imagePath": path as AnyObject,"username": username as AnyObject,"thread": thread as AnyObject,"date": dateString as AnyObject]
            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        var post: [String:String] = posts[postKey] as! [String:String]
                        postArray.append(Post(id: postKey, username: post["username"]!, postImagePath: post["imagePath"]!, thread: post["thread"]!, dateString: post["date"]!, read: ids.contains(postKey)))
                    }
